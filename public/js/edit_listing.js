...
$(document).ready(
	function initialize() {
		...
		map = new google.maps.Map(document.getElementById("edit_listing"), myOptions);
	...
	$('#edit_vineyard #continent').live('change', function () {
		...
		if (continent == '0') {
			...
			$('#ass_region').html('<option>Select country...</option>');
			$.ajax({
...
//		}	
		...
		$(".wines-grape-multiple").multiselect({
			noneSelectedText: 'Select Grapes',
			selectedList: 25
		}).multiselectfilter();
